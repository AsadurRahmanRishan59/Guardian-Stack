// app/api/master-admin/user/filter-options/route.ts
import { NextRequest } from 'next/server';

import { proxyToBackend } from '@/lib/api/proxy';

export async function GET(request: NextRequest) {
  return proxyToBackend(request, '/api/masteradmin/users/filter-options', { requireAuth: true });
}
...
//         const url = `${SPRING_BOOT_URL}/api/admin/user/filter-options${queryString ? `?${queryString}` : ''}`;
