...
export function useGetUserById(userId?: number) {
    return useQuery({
        queryKey: ["masterAdminUser", userId],   // consistent key prefix
        queryFn: async () => {
        ...
        enabled: !!userId,
        // Cache for 2 minutes — prevents refetch every time the modal re-renders
        // or re-opens for the same user. Mutations invalidate this key directly.
        staleTime: 2 * 60 * 1000,
        refetchOnWindowFocus: false,
        retry: 1,   // only retry once — modal errors should surface quickly
    });
}
...
export function useUpdateUser() {
    ...
    return useMutation({
        ...
        onSuccess: (res, { userId }) => {
            queryClient.invalidateQueries({ queryKey: ["masterAdminUserView"] });
            // Use the same key prefix as useGetUserById above
