...
function TimelineNode({
  ...
  return (
    <div className="relative pl-7 group" onClick={onClick}>
      {!isLastInGroup && (
        <div className="absolute left-[7px] top-5 bottom-0 w-0.5 bg-gradient-to-b from-border to-transparent" />
      )}
      <div className={cn("absolute left-0 top-4 h-3.5 w-3.5 rounded-full transition-all duration-150 z-10", dotColor)} />
      ...
      >
        <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
          <span className="font-mono text-xs font-bold text-blue-400">#{item.revisionNumber}</span>
        ...
        <div className="font-mono text-[10px] text-muted-foreground/35 truncate">{item.email}</div>
      </button>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-2 pt-2 pl-7">
      {Array.from({ length: 8 }).map((_, i) => (
        <Skeleton key={i} className="h-[88px] w-full rounded-lg" style={{ opacity: 1 - i * 0.1 }} />
      ))}
    </div>
  );
}
...
export function TimelineRail({ items, isLoading, selectedItem, onSelect }: TimelineRailProps) {
  ...
  // ✅ FIX: track how many times we've seen each userId so the header key is
  // always unique even when the same user appears in multiple page-interleaved
  // groups (e.g. after sorting changes).
  ...
  items.forEach((item, i) => {
    if (item.userId !== lastUserId) {
      ...
      // Key = "header-{userId}-{occurrence}" — unique even for repeated users
      nodes.push(
        <UserGroupHeader
          key={`header-${item.userId}-${count}`}
          email={item.email}
        />
      );
    }
